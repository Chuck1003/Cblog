...
router.get('/reg',function(req,res){
    res.render('reg',{title:"用户注册",pagename:'reg',message: req.session.error || ''});
});
...
router.get('/login',function(req,res){
    res.render('login',{title:"用户登陆",pagename:'login',message: req.session.error || ''});
});
...
router.get('/delete',function(req,res){
    var _selId = req.query.id;

    Post.remove({_id: _selId},function(err,posts){
        if(err){
            req.session.message=err.message;
            return res.redirect('/');
        }
